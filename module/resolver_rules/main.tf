...
#data "aws_vpc" "authoritative_use1" {
...
#data "aws_vpc" "authoritative_use2" {
...
#data "aws_route53_resolver_endpoint" "authoritative_inbound" {
...
#data "aws_route53_resolver_endpoint" "authoritative_outbound" {
#provider = aws.net_prod
#filter {
#name   = "DIRECTION"
#values = ["OUTBOUND"]
...
#data "aws_route53_resolver_endpoint" "authoritative_use1_inbound" {
#provider = aws.net_prod_use1
#filter {
#name   = "DIRECTION"
#values = ["INBOUND"]
...
#data "aws_route53_resolver_endpoint" "authoritative_use2_inbound" {
#provider = aws.net_prod_use2
#filter {
#name   = "DIRECTION"
#values = ["INBOUND"]
...
#data "aws_route53_resolver_endpoint" "authoritative_use1_outbound" {
#provider = aws.net_prod_use1
#filter {
#name   = "DIRECTION"
#values = ["OUTBOUND"]
...
#data "aws_route53_resolver_endpoint" "authoritative_use2_outbound" {
#provider = aws.net_prod_use2
#filter {
#name   = "DIRECTION"
#values = ["OUTBOUND"]
...
resource "aws_route53_resolver_rule" "rr_fwd_inbound" {
  ...
  resolver_endpoint_id = data.aws_route53_resolver_endpoint.authoritative_outbound.resolver_endpoint_id

  dynamic "target_ip" {
    for_each = { for k, v in data.aws_route53_resolver_endpoint.authoritative_inbound.ip_addresses : k => v if each.value.target_ips[0] == "inbound" }
...
resource "aws_route53_resolver_rule" "rr_fwd_ext_outbound_use1" {
  ...
  name                 = var.forward_rules.outbound[each.key].name
  rule_type            = (try(var.forward_rules.outbound[each.key].rule_type, null) != null) ? var.forward_rules.outbound[each.key].rule_type : "FORWARD"
  resolver_endpoint_id = data.aws_route53_resolver_endpoint.authoritative_use1_outbound.resolver_endpoint_id
...
resource "aws_route53_resolver_rule" "rr_fwd_ext_outbound_use2" {
  ...
  name                 = var.forward_rules.outbound[each.key].name
  rule_type            = (try(var.forward_rules.outbound[each.key].rule_type, null) != null) ? var.forward_rules.outbound[each.key].rule_type : "FORWARD"
  resolver_endpoint_id = data.aws_route53_resolver_endpoint.authoritative_use2_outbound.resolver_endpoint_id
